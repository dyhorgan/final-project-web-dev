...
const profileReducer = (state = {username: "", profiles: [], _id: "", imageUrl: "", bio: "", email: "", phone: "", otherProfile:{username: ""}}, action) => {
  switch(action.type){
    case CREATE_PROFILE:
      return {...state, username: action.newProfile.username, _id: action.newProfile._id};
    case FIND_PROFILE:
      if(action.profile){
         return {...state, username: action.profile.username, _id: action.profile._id, imageUrl: action.profile.imageUrl, bio: action.profile.bio, email: action.profile.email, phone: action.profile.phone};
      }else{
            break;
      }
